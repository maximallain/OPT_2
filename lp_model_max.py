...
def model(input_path='input_'+n+'.txt', grid_unit=1):
    ...
    x_min, x_max, y_min, y_max, [r1, r2], [
        p1, p2], positions_list = read_map(input_path)

    model = Model('optim')

    # Cadrillage
    c_x_list = [
        x_min+k*grid_unit for k in range(0, ceil((x_max-x_min)/grid_unit)+1)]
    c_y_list = [
        y_min+k*grid_unit for k in range(0, ceil((y_max-y_min)/grid_unit)+1)]
    ...
    for u in [1, 2]:
        for i in c_x_list:
            for j in c_y_list:
                variable_dict["c-" + str(u) + "-" + str(i) + "-" + str(j)] = model.addVar(
                    "c-" + str(u) + "-" + str(i) + "-" + str(j), vtype="INTEGER")
